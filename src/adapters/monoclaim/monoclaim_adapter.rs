...
use crate::sd_algorithms::monoclaim::monoclaim::MonoclaimCredentialsInstance;
...
impl Adapter for MonoclaimAdapter {
    ...
    fn issue_vc(&self, raw_vc: &Map<String, Value>) -> Result<(Map<String, Value>, String), String> {
        MonoclaimCredentialsInstance::issue_vc(raw_vc, &self.issuer_private_key)
    }

    fn verify_vc(&self, vc: &Map<String, Value>) -> Result<(), String> {
        MonoclaimCredentialsInstance::verify_vc(vc, &self.issuer_public_key)
    }

    fn issue_vp(&self, vc: &Map<String, Value>, disclosures: &Vec<String>) -> Result<(Map<String, Value>, String), String> {
        MonoclaimCredentialsInstance::issue_vp(vc, disclosures, &self.holder_private_key)
    }

    fn verify_vp(&self, vp_jwt: &String) -> Result<(), String> {
        MonoclaimCredentialsInstance::verify_vp(vp_jwt, &self.issuer_public_key, &self.holder_public_key)
    }
